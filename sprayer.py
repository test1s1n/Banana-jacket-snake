...
def parseInputs(envMethod: bytes, envURL: bytes, envHeaders: bytes, envBody: bytes, aesKey: bytes) -> typing.Tuple[bytes, bytes, dict, bytes]:
...
def makeRequest(method: bytes, url: bytes, headers:dict=None, body:bytes=None) -> typing.Tuple[int, dict, bytes, str]:
    try:
...
if respErr != None:
    ...
    # Prints are chunked in order to avoid GitHub issues and slowness of printing large strings
